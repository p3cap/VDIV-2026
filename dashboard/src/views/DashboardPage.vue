		...
		<main class="dash-grid">
			...
			<div class="card status-card" :class="statusAccent">
				...
				<div class="status-info">
					...
					<span class="s-sub"
						>x:{{ dashboard?.rover_position?.x ?? 0 }} · y:{{
							dashboard?.rover_position?.y ?? 0
						}}</span
					>
				</div>
			...
			<div class="card battery-card">
				<div class="card-header">
					...
					<span class="card-value" :style="{ color: batteryColor }"
						>{{ dashboard?.rover_battery?.toFixed(1) ?? '—' }}%</span
					>
				</div>
				<div class="battery-arc-wrap">
					<svg viewBox="0 0 200 120" class="battery-svg">
						...
						<text
							...
							font-weight="700"
						>
							{{ Math.round(dashboard?.rover_battery ?? 0) }}
						</text>
						<text
							x="100"
							y="114"
...
function updateDist() {
	if (!uDist) return
	uDist.setData([Float64Array.from(hTime), Float64Array.from(hDist)])
}
function updateEnergy() {
	if (!uEnergy) return
	uEnergy.setData([Float64Array.from(hTick), Float64Array.from(hProd), Float64Array.from(hCons)])
}
...
function reflow() {
	...
	if (uEnergy && energyRef.value) {
		uEnergy.setSize({
			width: energyRef.value.clientWidth,
			height: energyRef.value.clientHeight,
		})
	}
	drawScatter()
}
