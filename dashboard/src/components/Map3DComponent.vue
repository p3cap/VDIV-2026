...
const S = {
  ...
  mTile: null, mBarrier: null, mGold: null, mIce: null,
  mGreen: null, mGreenDark: null, mStart: null, mPole: null, mMined: null,
};
...
function makeGold(gx, gy) {
  const g = new Group();
  const c1 = new Mesh(S.gGold, S.mGold); c1.position.set(-0.2, 0.34, -0.14); c1.rotation.set(0.4, 0.7, 0); c1.scale.y = 1.5;
  const c2 = new Mesh(S.gGold, S.mGold); c2.position.set(0.18, 0.27, 0.16);  c2.rotation.set(-0.3, 1.2, 0.2); c2.scale.y = 1.2;
  g.add(c1, c2); g.position.copy(toWorld(gx, gy, 0)); return g;
}
function makeIce(gx, gy) {
  const g = new Group();
  const s1 = new Mesh(S.gIceCone, S.mIce); s1.position.set(-0.18, 0.33, 0.10); s1.rotation.z = -0.2;
  const s2 = new Mesh(S.gIceCone, S.mIce); s2.position.set(0.18, 0.27, -0.14); s2.rotation.z = 0.25;
  g.add(s1, s2); g.position.copy(toWorld(gx, gy, 0)); return g;
}
function makeGreen(gx, gy) {
  const g    = new Group();
  const stem = new Mesh(S.gGreenStem, S.mGreenDark); stem.position.y = 0.18;
  const cap  = new Mesh(S.gGreenCap,  S.mGreen);     cap.position.y  = 0.50; cap.scale.y = 0.60;
  g.add(stem, cap); g.position.copy(toWorld(gx, gy, 0)); return g;
}
function makeStart(gx, gy) {
  ...
  g.add(pole, flag); g.position.copy(toWorld(gx, gy, 0)); return g;
}
...
function buildRover() {
  ...
  const chassis = new Mesh(new BoxGeometry(1.0, 0.18, 1.3), sm(C.rover, { roughness: 0.5 })); chassis.position.y = 0.42; root.add(chassis);
  const body    = new Mesh(new BoxGeometry(0.80, 0.30, 0.86), sm(C.rover)); body.position.y = 0.64; root.add(body);

  const panel = new Mesh(new BoxGeometry(1.42, 0.04, 0.68),
    new MeshStandardMaterial({ color: C.solar, roughness: 0.3, metalness: 0.7, emissive: new Color("#001a55"), emissiveIntensity: 0.3 }));
  panel.position.y = 0.83; rover.solarPanel = panel; root.add(panel);
  ...
  for (const [wx, wz] of [[-0.57,-0.46],[-0.57,0],[-0.57,0.46],[0.57,-0.46],[0.57,0],[0.57,0.46]]) {
    ...
    wg.add(tyr, hub); wg.position.set(wx, 0.22, wz); rover.wheels.push(wg); root.add(wg);
  }

  const mast = new Mesh(new CylinderGeometry(0.03, 0.03, 0.52, 6), sm("#aaaaaa")); mast.position.set(0, 1.00, -0.30); root.add(mast);
  const head = new Mesh(new BoxGeometry(0.13, 0.11, 0.11), sm("#222222", { metalness: 0.5 })); head.position.set(0, 1.30, -0.30); root.add(head);

  rover.statusLight = new Mesh(new SphereGeometry(0.07, 6, 5),
  ...
  rover.drillGroup = new Group(); rover.drillGroup.visible = false;
  const arm = new Mesh(new CylinderGeometry(0.03, 0.03, 0.46, 5), sm("#888888")); arm.rotation.z = Math.PI / 2; arm.position.set(0.33, 0.30, 0.50);
  const bit = new Mesh(new ConeGeometry(0.07, 0.18, 6), sm("#555555", { metalness: 0.7 })); bit.rotation.z = -Math.PI / 2; bit.position.set(0.58, 0.30, 0.50);
  rover.drillGroup.add(arm, bit); root.add(rover.drillGroup);

  rover.root = root; return root;
}
...
function buildPath() {
  ...
  pathTubeMat  = new MeshLambertMaterial({
    color: C.path, emissive: new Color(C.path), emissiveIntensity: 0.65,
...
let lastTs = 0;
function markDirty() { if (!frameId) frameId = requestAnimationFrame(renderLoop); }
...
function renderLoop(ts) {
  ...
  if (keys.size > 0) {
    ...
    if (keys.has("w") || keys.has("arrowup")) {
      cam.targetX -= sinA * panSpd; cam.targetZ -= cosA * panSpd;
    }
    ...
    if (keys.has("a") || keys.has("arrowleft")) {
      cam.targetX -= cosA * panSpd; cam.targetZ += sinA * panSpd;
    }
...
function screenToWorldPan(dx, dy) {
  ...
  const fwd = new Vector3(); camera.getWorldDirection(fwd);
  const right = new Vector3().crossVectors(fwd, new Vector3(0, 1, 0)).normalize();
  fwd.y = 0; fwd.normalize();
  const scale = cam.radius * 0.003;
  return [
    (-dx * right.x + dy * fwd.x) * scale,
    (-dx * right.z + dy * fwd.z) * scale,
  ];
}
...
function onPointerMove(e) {
  if (cam.panDragging) {
    ...
    cam.vPanX = wx; cam.vPanZ = wz;
    cam.targetX += wx; cam.targetZ += wz;
    cam.panLastX = e.clientX; cam.panLastY = e.clientY;
    camApply();
  }
  if (cam.orbitDragging) {
    ...
    cam.theta += cam.vTheta; cam.phi += cam.vPhi;
    cam.orbitLastX = e.clientX; cam.orbitLastY = e.clientY;
    camApply();
  }
}
...
<template>
  <div ref="canvasWrap" class="canvas-wrap">
    <div class="hint">
      ...
      <span>⌨ WASD/↑↓←→: pan &nbsp;|&nbsp; Q/E: rotate &nbsp;|&nbsp; +/−: zoom &nbsp;|&nbsp; R: reset</span>
    </div>
  </div>
</template>
